...
class DNN_05(DNN):
    def __init__(self, numClasses, shape, batchSize = 50, relativeStepSize = 0.001, momentum = 0.9, regularization=0.0):
        ...
        d2 -= 2
        d3 -= 2
        self.layers.append(DNNLayer(DNNLayerType.FFFRELU, (batchSize, d1, d2, d3), None, self.layers[-1]))
        self.layers.append(DNNLayer(DNNLayerType.CONVO, (batchSize, 16, d2-2, d3-2), (d1, 3, 3, 16), self.layers[-1]))
        d1 = 16
        d2 -= 2
        d3 -= 2
        self.layers.append(DNNLayer(DNNLayerType.FFFRELU, (batchSize, d1, d2, d3), None, self.layers[-1]))
        self.layers.append(DNNLayer(DNNLayerType.CONVO, (batchSize, 256, d2-6, d3-6), (d1, 7, 7, 256), self.layers[-1]))
        d1 = 256
        d2 -= 6
        d3 -= 6
